# M3508 电机驱动测试项目（基于 STM32F427IIH6）

## 介绍
该项目基于 RoboMaster A 型开发板，使用 STM32F427IIH6 微控制器，通过 CAN 总线控制无刷电机 **M3508** 及其电调 **C620**。项目结构清晰，模块化设计便于维护和扩展。

## 更新日志
- 2025.12.03 - 新建 `/User_doc/反思.md`
- 2025.11.24 - 修复串口调参, 目前可以用串口调了, `output`是范围[-16384, 16384]的电流映射值

## 项目结构
- **Core/**：CubeMX 自动生成的 STM32 核心驱动代码。
- **User_alg/**：软件算法实现，如 PID 控制器。
- **User_drv/**：驱动封装，包括 CAN 和串口通信驱动。
- **User_dvc/**：设备抽象层，实现对电机的控制逻辑。
- **User_tsk/**：任务调度，包括中断回调和主循环逻辑。

## 外设说明
### CAN 总线
- 使用 **CAN1**，高速模式，波特率 **1 Mbps**。
- 仅使用 **FIFO0**，过滤器为 32 位，仅接收 **CAN ID 0x200~0x208** 的数据。

### UART 串口
- 使用 **UART2**，波特率 **115200 bps**，数据格式为 **8N1**。
- **MCU 发送数据包格式**：`data:%f,%f,%f,%f,%f,%f\n`，其中 `%f` 表示浮点型数据。
- **上位机接收数据包格式**：`Kx%.1f\n`，其中：
  - `K` 为包头（注意要大写）；
  - `%.1f` 为具体数据，可自定义保留小数；
  - `x` 要写 `p`、`i`、`d`、`t` 中的一个，分别表示 `Kp`、`Ki`、`Kd` 和目标值；
  - `\n` 包尾。

## 使用说明
1. 看清楚用的是哪个外设，别接线接错了；
2. 已在 STM32F103 平台完成硬件在环测试，STM32F427IIH6 移植完成。

## 引脚图
![引脚图](./引脚图.png)

## 参与贡献
1. 请先联系项目作者（RoboPioneer 下位机小登）；
2. Fork 本仓库；
3. 创建新分支；
4. 提交代码；
5. 创建 Pull Request。

## 其他说明
- 本项目为练手性质，非正式生产项目。
- 由于实验机会有限，开发进度可能较慢。

## 许可协议
本项目采用开源协议（请参考项目根目录的 LICENSE 文件）。

---
> 东北大学秦皇岛分校RoboPioneer下位机某小登的练手项目 ^_^  
> 该项目由 [Gitee](https://gitee.com/liuyihui1/m3508-driver-test-stm32f427iih6) 托管，GitHub上有克隆，但不会频繁同步更新，请以Gitee仓库为准