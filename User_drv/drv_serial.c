...
{
	return HAL_UART_Transmit_DMA(&huart3, serial_tx_packet, SERIAL_PACKET_SIZE);
}
 ...
 * @warning 不得超过100字符
 */
HAL_StatusTypeDef UART3_printf(char *format, ...)
{
	char String[100];				//定义字符数组
	va_list arg;					//定义可变参数列表数据类型的变量arg
	va_start(arg, format);			//从format开始，接收参数列表到arg变量
	vsprintf(String, format, arg);	//使用vsprintf打印格式化字符串和参数列表到字符数组中
	va_end(arg);					//结束变量arg
	return Serial_Printf_DMA(&huart3, String, strlen(String));		//串口发送字符数组（字符串）
}
...
void Serial_RxEventCallback(UART_HandleTypeDef *huart, uint16_t Size)
{
	...
	{
        serial_rx_flag = 1;
		serial_rx_packet[Size] = '\0';		

		// 重新开启DMA接收
        HAL_UARTEx_ReceiveToIdle_DMA(&huart3, serial_rx_packet, SERIAL_PACKET_SIZE);
        __HAL_DMA_DISABLE_IT(huart3.hdmarx, DMA_IT_HT);		// 关闭传输一半中断
	}
}
